...
const Settings = () => {
  ...
  const setSemDate = async () => {
    ...
    if(!newDate.semester){
      alert("Please choose the semester type.");
      return;
    }

    if (!newDate.startDate) {
      alert("Please fill in start date of the semester.");
      return;
    }
    ...
    try {
      ...
      for (let week = 1; week <= 19; week++) {
        ...
        if (week == 19){
          ...
          alert('Semester date updated successfully!');
          fetchWeekData();
        }
      }
    } catch (error) {
  ...
  const handleAddItem = async () => {
    ...
    if (!firstItem.docId) {
      console.error("docId is missing in the first item of itemData.");
      return;
    }
